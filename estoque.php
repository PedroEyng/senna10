...
if (isset($_POST['update_product'])) {
    ...
    if ($update_query->execute()) {
        ...
        // Processamento do upload de novas imagens
        $upload_dir = 'uploads/';
        if (!is_dir($upload_dir)) {
            mkdir($upload_dir, 0777, true);  // Cria a pasta, se não existir
        }
        ...
        if (!empty($imagens)) {
            ...
            $delete_images_query = $conexao->prepare("DELETE FROM imagem_produtos WHERE produto_id = ?");
            $delete_images_query->bind_param("i", $update_p_produto_id);
            $delete_images_query->execute();

            // Inserir novas imagens
            foreach ($imagens as $imagem_caminho) {
                $insert_image_query = $conexao->prepare("INSERT INTO imagem_produtos (produto_id, caminho_imagem) VALUES (?, ?)");
                $insert_image_query->bind_param("is", $update_p_produto_id, $imagem_caminho);
                $insert_image_query->execute();
            }
        }

        $message[] = 'Produto e imagens atualizados com sucesso';
    } else {
...
<?php if ($product_data): ?>
    ...
    <div class="edit-form-container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <!-- Campos para upload de imagens -->
            <input type="file" name="p_imagem1" accept="image/png, image/jpg, image/jpeg" class="box">
